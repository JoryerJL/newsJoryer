...
- Python `3.13.15`, Django `5.2.17`, PostgreSQL `17.11`, `uv` y `python-decouple`.
...
- No crear SPA, API separada, microservicios, Celery, Redis, Docker ni nuevas dependencias sin decisión explícita. Tailwind CSS `4.3` es la excepción aprobada: se compila en build con `@tailwindcss/cli`, sin CDN ni runtime de Node en EC2.
- Settings separados: `base`, `development`, `test` y `production`.
- Usar `uv` para inicializar el proyecto e instalar dependencias; usar los generadores del framework o CLI (`uv init`, `uv add`, `django-admin startproject`, `manage.py startapp`, npm/CLI) en vez de crear manualmente los artefactos que generan. La configuración requerida que no genere una herramienta se mantiene explícita y manual.
...
2. Codex/Terra implementa backend y pruebas.
3. Cursor implementa frontend con las referencias Stitch asignadas.
4. Codex/Terra crea `docs/qa/<phase>-functional-test-plan.md` sobre el producto integrado.
...
- El plan QA se crea después de integrar backend y frontend; cubre éxito, validación, permisos,
  persistencia, errores/estados relevantes y regresiones.
